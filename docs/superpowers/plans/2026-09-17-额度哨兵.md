...
describe("probability（离散风险率：等待 d 天后 24h 内重置比例）", () => {
  it("常规样本：interval (d, d+1] 计分子，> d 计分母", () => {
    // intervals: [1,2,3,5,10]; d=2 → 分母 {3,5,10}=3，分子 {3}=1 → 33%
    const r = probability([1, 2, 3, 5, 10], 2);
  ...
  it("分层标签：低/中/高", () => {
    ...
    expect(probability([2, 2.5, 3, 5, 10], 2).label).toBe("高");    // 4/5=80%
    expect(probability([4.5, 4.6, 4.8, 5, 10], 4.5).label).toBe("中"); // 3/5=60%→中
  });
